...
impl BlockProducer {
    ...
    pub async fn handle_event(&mut self, event: ChainEvent) -> Result<()> {
        if TestMode::Enable == self.tests_control.mode() {
            match self.tests_control.payload().await {
                ...
                Some(TestModePayload::None) => self.tests_control.none().await?, // Produce block
    ...
    pub async fn produce_next_block(
        ...
        if TestMode::Enable == self.tests_control.mode() {
            if let Some(TestModePayload::BadBlock { .. }) = self.tests_control.payload().await {
                block = self.tests_control.bad_block(block).await?;
            }
        }
